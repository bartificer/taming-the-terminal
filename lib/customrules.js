...
const fixCodeBlock = {
  ...
  replacement: function (content, node, options) {
    ...
    let language = node.querySelector('.crayon-language')
      ? node.querySelector('.crayon-language').textContent
      : 'bash';
 ...
 *
 * Since most of these snippets are shell add a default bash syntax highlighting
 */
...
/**
 * fixFormattedCodeSnippets
 *
 * Sometimes there are <p><code>[formatted code snippet]</code></p>
 * formatted code snippets.
 * Having them converted to backticks prevents the snippet in between
 * to be formatted which looks odd.
 *
 * For now we just copy it as is
 */
const fixFormattedCodeSnippets = {
  filter: function (node) {
    return node.nodeName === 'P' && node.firstChild.nodeName === 'CODE';
  },
  replacement: function (content, node, options) {
    // console.log('fixFormattedCodeSnippets: ' + content);
...
const fixPodcastLink = {
  ...
  replacement: function (content, node, options) {
    ...
    if (ccatpLink.length > 0) {
      ...
      const match = re.exec(podcastLink);

      episodeNumber = match && match.length > 1 ? re.exec(podcastLink)[1] : '';
    }

    // assume more than one audio link, because 27 does have 2
    const audioTags = node.getElementsByTagName('AUDIO');
    let audioControls = '';
    let downloadLink = '';
    let i = 0;
    for (const tag of audioTags) {
      const audioLink = tag.getElementsByTagName('A')[0].getAttribute('href');
      audioControls += `<audio controls='1' data-external='1' src="${audioLink}">Your browser does not support HTML 5 audio 🙁</audio>\n\n`;
      const index = audioTags.length > 1 ? `(${i + 1})` : '';
      downloadLink += `You can also <a href="${audioLink}?autoplay=0&loop=0&controls=1" >Download the MP3 ${index}</a>\n\n`;
      i++;
    }
...
const fixInstalmentRefs = {
  filter: function (node) {
    const matching =
      ...
      node
        .getAttribute('href')
        .match(/\/www.bartbusschots.ie.*\/ttt-([0-9]+)-of-x.*$/);
    return matching;
  },
  replacement: function (content, node, options) {
    // build the original link, because it looks like content
    ...
    const link = node.getAttribute('href');
    const oldLink = `[${node.textContent}](${link})`;
    const re = /\/www.bartbusschots.ie.*\/ttt-([0-9]+)-of-x.*$/;
    const match = re.exec(link);
    if (match.length > 1) {
      // we found a link to a different instalment

      const newLink = `[${node.textContent}](https://bartificer.net/ttt${
...
const fixImages = {
  filter: function (node) {
    const matching =
      ...
      node.getAttribute('src').match(/\/www.bartbusschots.ie.*\/.*.png$/);
    return matching;
  },
  replacement: function (content, node, options) {
    let alt = node.alt || '';

    // alt text is shown as caption
    // so remove it when the alt text is something
    // like Screen shot <date time>

    alt = alt.replace(/Screen ?Shot \d\d\d\d \d\d \d\d at \d\d \d\d \d\d/, '');
...
module.exports = [
  ...
  {
    name: 'fixFormattedCodeSnippets',
    rule: fixFormattedCodeSnippets
  }
];
