...
====
Please refer to xref:ttt38[ttt38] in which it is explained that `screen` was deprecated in all Red Hat variants of Linux in 2019, and replaced with the much more powerful `tmux`.
====
...
image::qrcodes/TTT_36.png[QR code, align='left']
...
This will open your crontab with your system's default text editor (probably `vi`, which we learned about in xref:ttt11[instalment 11]).
...
If email doesn't "just work" for you, my advice would be to change tack and use stream redirection (as described in instalments xref:ttt15[15] and xref:ttt16[16]) instead.
...
As we learned in xref:ttt12[instalment 12], you can see the content of your environment in a regular shell with the command `env`.
