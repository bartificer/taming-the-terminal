...
In the xref:ttt19[previous instalment], we looked at using `egrep` to search for a particular piece of text in a stream or file.
...
image::qrcodes/TTT_20.png[QR code, align='left']
...
To use the current folder as the base of your search use a path of `.` (which always means '`the current folder`' as we learned in xref:ttt04[instalment 4]).
...
To avoid this, run the command with `sudo`, or, if you want to just ignore the errors, redirect `STDERR` to `/dev/null` with `2>/dev/null` like we learned in xref:ttt16[instalment 16].
...
Secondly, by default `-regex` and `-iregex` use Basic POSIX Regular Expressions (BREs), rather than the newer Extended POSIX Regular Expressions (EREs) we learned about in instalments xref:ttt17[17] and xref:ttt18[18].
