...
In the <<ttt08,previous instalment>>, we looked at how Unix-like operating systems like Linux and Mac OS X represent processes.
...
ifndef::backend-pdf[]
+++<audio controls='1' src="https://media.blubrry.com/tamingtheterminal/archive.org/download/TTT09ControllingProcesses/TTT_09_Controlling_Processes.mp3">+++Your browser does not support HTML 5 audio 🙁+++</audio>+++
endif::[]

Scan the QRcode to listen on a different device

image::./assets/qrcodes/TTT_09.png[QRcode, align='left']

You can
ifndef::backend-pdf[also]
https://media.blubrry.com/tamingtheterminal/archive.org/download/TTT09ControllingProcesses/TTT_09_Controlling_Processes.mp3?autoplay=0&loop=0&controls=1[download the MP3]
...
When you pass `open` one or more files as arguments it will open those files using the default app for their file type, and as an added bonus, it will automatically background and apps it starts, so you don't even have to remember the `&`!
...
Rather than looking up it's PID, let's now exit it with the `killall` command:
...
Think of it this way, it's basic good manners to ask the process the please leave before you call the bouncers over to eject it!
