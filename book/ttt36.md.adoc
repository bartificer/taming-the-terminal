...
This will open your crontab with your system's default text editor (probably `vi`, which we learned about in <<ttt11.adoc,instalment 11>>).
...
If email doesn't _just work_ for you, my advice would be to change tack, and use stream redirection (as described in instalments <<ttt15.adoc,15>> and <<ttt16.adoc,16>>) instead.
...
As we learned in <<ttt12.adoc,instalment 12>>, you can see the content of your environment in a regular shell with the command `env`.
