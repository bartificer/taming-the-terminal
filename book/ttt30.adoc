...
[NOTE]
====
This instalment was initially written, and the matching podcast episode recorded, in October 2015.

In February 2021 some sections of this instalment specific Mac OS X/macOS were updated to reflect signifficant changes in Apple's handling of SSH keys. The most signifficant change Apple made is in the SSH Agent, which is described in <<ttt37,TTT37>>, but Apple also added support for the `ssh-copy-id` command described in this instalment.

The podcast episode for this instalment was not updated in February 2021, but the update to `ssh-copy-id` was mentioned in a suplimental podcast episode for <<ttt37,TTT37>>.
====
...
_**Update -- February 2021:** this section has been updated to reflect changes in macOS as opposed to Mac OS X._
...
[NOTE]
.Installing `ssh-copy-id` on Mac OS X 10.11 El Capitan and Older
====
When Apple re-named Mac OS X to macOS they updated the version of Open SSH included with the OS to one that includes the `ssh-copy-id` command. Before Apple made this move the Mac did not ship with `ssh-copy-id`, but the open source community stepped in to fill the void. As of February 2021 there are at least three options available to Mac OS X users — (https://formulae.brew.sh/formula/ssh-copy-id)[a Hombrew formula], a https://ports.macports.org/port/ssh-copy-id/summary[MacPorts port], and a https://github.com/beautifulcode/ssh-copy-id-for-OSX/[simple script].
====
...
The version of SSH that ships with Apple's macOS (formerly Mac OS X)has support for Apple's secure keychain.
This is a secure vault the Mac uses to store the passwords you save in all sorts of apps, including Mail.app and Safari. It's possible to securely store your SSH Key passphrases in Apple's Keychain, and once you do, you can use your key without entering your passphrase each time you use it.

On older versions of Apple's operating system the integration with the Keychain was integreated into the Mac GUI and enabled by default. When you used an SSH key a popup window would appearing asking for your password and offering to save it to your Keychain. If you ticked the box to save the password, you could then log in with out entering your password in future. This was extremely convenient, but very much non-standard behaviour.
...
Regardless of whether you're using macOS or Linux, you now need to use the SSH Agent to manage your SSH keys. This agent is discussed in detail in <<ttt37,TTT37>>.

== Advantages to Key-based Authentication

* *Convenience* -- with `ssh-agent` securely storing the password for your private key, you can safely use SSH without having to enter a password.
