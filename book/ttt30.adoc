...
In xref:ttt29[the previous instalment], we saw how we can use SSH to execute a single command on a remote computer, or, to get a command shell on a remote computer.
...
In February 2021 some sections of this instalment specific Mac OS X/macOS were updated to reflect significant changes in Apple's handling of SSH keys. The most significant change Apple made is in the SSH Agent, which is described in xref:ttt37[TTT37], but Apple also added support for the `ssh-copy-id` command described in this instalment.

The podcast episode for this instalment was not updated in February 2021, but the update to `ssh-copy-id` was mentioned in a supplemental podcast episode for xref:ttt37[TTT37].
====
...
image::qrcodes/TTT_30.png[QR code, align='left']
...
====
When Apple re-named Mac OS X to macOS they updated the version of Open SSH included with the OS to one that includes the `ssh-copy-id` command. Before Apple made this move the Mac did not ship with `ssh-copy-id`, but the open source community stepped in to fill the void. As of February 2021 there are at least three options available to Mac OS X users — (https://formulae.brew.sh/formula/ssh-copy-id)[a Homebrew formula], a https://ports.macports.org/port/ssh-copy-id/summary[MacPorts port], and a https://github.com/beautifulcode/ssh-copy-id-for-OSX/[simple script].
====
...
Regardless of whether you're using macOS or Linux, you now need to use the SSH Agent to manage your SSH keys. This agent is discussed in detail in xref:ttt37[TTT37].
