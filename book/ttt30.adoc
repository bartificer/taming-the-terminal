...
[NOTE]
====
This instalment was initially written, and the matching podcast episode recoreded, in October 2015.
...
[NOTE]
.Installing `ssh-copy-id` on Mac OS X 10.11 El Capitan an Older
====
When Apple re-named Mac OS X to macOS they updated the version of Open SSH included with the OS to one that includes the `ssh-copy-id` command. Before Apple made this move the Mac did not ship with `ssh-copy-id`, but the open source community stepped in to fill the void. As of Febraury 2021 there are at least three options available to Mac OS X users — (https://formulae.brew.sh/formula/ssh-copy-id)[a Hombrew formula], a https://ports.macports.org/port/ssh-copy-id/summary[MacPorts port], and a https://github.com/beautifulcode/ssh-copy-id-for-OSX/[simple script].
====
