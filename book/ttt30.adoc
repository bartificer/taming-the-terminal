...
In February 2021 some sections of this instalment specific Mac OS X/macOS were updated to reflect significant changes in Apple's handling of SSH keys. The most significant change Apple made is in the SSH Agent, which is described in <<ttt37,TTT37>>, but Apple also added support for the `ssh-copy-id` command described in this instalment.

The podcast episode for this instalment was not updated in February 2021, but the update to `ssh-copy-id` was mentioned in a supplemental podcast episode for <<ttt37,TTT37>>.
====
