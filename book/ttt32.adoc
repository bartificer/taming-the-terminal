...
So far we've learned how to xref:ttt29[securely execute terminal commands on remote computers], how to xref:ttt31[securely copy files across the network using SSH], and how to xref:ttt30[add both security and convenience] to both those operations with SSH key pairs.

As we saw in xref:ttt31[the previous instalment], SSH's ability to provide a secure connection between two computers can be used in many different ways.
...
image::qrcodes/TTT_32.png[QR code, align='left']
