...
http://media.blubrry.com/tamingtheterminal/archive.org/download/TTT32SSHTunneling/TTT_32_SSH_Tunneling.mp3?autoplay=0&loop=0&controls=1[download the MP3]
...
Dynamic Port Forwarding makes it possible for any app that can use the standard http://en.wikipedia.org/wiki/SOCKS[SOCKS protocol] to route traffic through an SSH connection, that includes apps like web browsers, chat clients, and email clients.
