...
In <<ttt25.adoc,the previous instalment>> we looked at how IP subnet works.
...
As we learned in <<ttt24.adoc,Instalment 24>>, `FF:FF:FF:FF:FF:FF` is a special MAC address that's used to send a packet to every device on an ethernet network.
