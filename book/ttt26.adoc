...
In xref:ttt25[the previous instalment], we looked at how IP subnet works.
...
image::qrcodes/TTT_26.png[QR code, align='left']
...
As we learned in xref:ttt24[Instalment 24], `FF:FF:FF:FF:FF:FF` is a special MAC address that's used to send a packet to every device on an Ethernet network.
