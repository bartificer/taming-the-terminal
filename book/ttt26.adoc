...
pass:m,a[<audio controls='controls' src="{url-mp3}/TTT26DHCP/TTT_26_DHCP.mp3">Your browser does not support HTML 5 audio</audio>]
endif::[]
...
You can also {url-mp3}/TTT26DHCP/TTT_26_DHCP.mp3[download the MP3]
...
Assuming the requested IP is still free, the server will broadcast back a DHCP ACK (acknowledgemnt) packet confirming to the client that it can use those details.
