...
In the xref:ttt19[first instalment] on `find`, we learned how to search for text within files and streams using `egrep`.
In xref:ttt20[the second], we learned to search for files based on all sorts of criteria with the `find` command.
...
image::qrcodes/TTT_21.png[QR code, align='left']
...
As we learned in xref:ttt06[instalment 6], we can make a file executable with the command:
...
We also know from xref:ttt15[instalment 15] that we can use the `wc` command with the `-l` flag to count the number of lines in  one or more files:
