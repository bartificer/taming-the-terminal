...
In the <<ttt19.adoc,first instalment>> we learned how to search for text within files and streams using `egrep`.
In <<ttt20.adoc,the second>> we learned to search for files based on all sorts of criteria with the `find` command.
...
As we learned in <<ttt6.adoc,instalment 6>>, we can make a file executable with the command:
...
We also know from <<ttt15.adoc,instalment 15>> that we can use the `wc` command with the `-l` flag to count the number of lines in a one or more files:
