...
E.g.
MacPorts installs all its binaries in `+/opt/local/...+`, so when I install http://www.imagemagick.org/[Image Magick] via MacPorts the `convert` binary is installed to `/opt/local/bin/convert`.
====
