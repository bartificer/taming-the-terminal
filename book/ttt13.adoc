...
In xref:ttt12[the previous instalment], we introduced the concept of the command shell environment, and we looked in detail at how shell and environment variables work.
...
image::qrcodes/TTT_13.png[QR code, align='left']
...
[NOTE]
====
This instalment assumes you're using Bash, if you're on macOS Catalina or later you're probably using Zsh, see xref:introduction[the Introduction] for more information on using Bash on newer Macs.
====
...
If you remember right back to xref:ttt02[the second instalment], we said that when entering commands on the command line, the first word is the command, and the other words (separated by spaces) formed the arguments to that command.
