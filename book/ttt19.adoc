...
In the previous two instalments (xref:ttt17[17] & xref:ttt18[18]) of this series, we learned how to represent patterns with regular expressions, or, to be more specific, with POSIX Extended Regular Expression (or EREs).
...
image::qrcodes/TTT_19.png[QR code, align='left']
...
There are two basic ways in which `egrep` can be used -- it can filter whatever is sent to via standard in (`STDIN` -- see xref:ttt15[part 15]) against a given pattern, or, it can search for lines matching a given pattern in one or more files.
