...
In xref:ttt15[the previous instalment], we introduced the concepts of streams and looked at how every process has references to three streams as part of their environment -- `STDIN`, `STDOUT` & `STDERR`.
...
image::qrcodes/TTT_16.png[QR code, align='left']
...
You can use multiple redirects in one command.
For example, you could use one redirect to send data from a file to a command, and another redirect to send the output to a different file.
