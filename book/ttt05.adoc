...
image::qrcodes/TTT_05.png[QR code, align='left']
...
If set on a folder it means the contents of the files and folders contained within the folder can be read, assuming the permissions masks further down the filesystem tree also allow that.
...
* `b` `c` `p` and `s` are also valid file types, but they are used to represent things like block devices and sockets rather than '`normal`' files, and we'll be ignoring them in this series.
