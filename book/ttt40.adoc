...
image::qrcodes/TTT_40.png[QR code, align='left']
...
Before we do that, we should make our commands just a little bit more robust by reusing the tip from the end of xref:ttt38[instalment 38] and using the `||` operator to reattach to an existing session with the desired name, or, create a new one:
...
We looked as aliases in xref:ttt14[instalment 14], but as a quick reminder — the command to create an alias in Bash (and ZSH) is `alias`, it takes the following form:
