...
Before we do that, we should make our commands just a little but more robust by re-using the tip from the end of instalment 38 and using the `||` operator to re-attach to an existing session with the desired name, or, create a new one:
