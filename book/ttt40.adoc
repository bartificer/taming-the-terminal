...
As a reminder, the command to create an alias in Bash (and ZSH) is `alias`, it takes the following form:
...
This series will now go back on hiatus for a little while, but the plan is to resurect it again in a few months when Apple release macOS 11 Big Sur. Why? Because that's an opportune moment to release a series of instalments on the topic of switching from Bash to ZSH. Since macOS Catalina the default shell on the Mac has been ZSH rather than Bash, with Bash available as an option. Since Catalina was such a bug-fest on launch many people have chosen not to upgrade all their machines (me included!), but I expect that with Big Sur's debut there will be a lot of Terminal Tamers suddently finding Bash's familar `$` command prompt replaced with ZSH's alien-feeling `%` command prompt!