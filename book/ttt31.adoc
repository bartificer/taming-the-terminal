...
In xref:ttt29[Part 29 of n], we learned how to use SSH to execute commands on a remote computer.
In xref:ttt30[the previous instalment], we learned how to add security and convenience to SSH connections using SSH key-pairs.
...
image::qrcodes/TTT_31.png[QR code, align='left']
...
====
In later versions of OSX (macOS Sierra and higher), you will not get the popup offering to save your passphrase to Apple Keychain. See xref:ttt37[TTT37] under "SSH Agent on OS X & macOS" for the new method to store your passphrase in Keychain.
====
