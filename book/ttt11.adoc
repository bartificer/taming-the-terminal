...
Remember that the only way out of a `tail -f` is with kbd:[Control+c].
...
Once in `nano`, you'll see a cursor where you can start to type, and along the bottom a list of possible commands with their key combinations next to them (remember, `^` is short-hand for the kbd:[Control] key).
...
After hitting kbd:[Control+o] `nano` will then show you the path it's about to write the file to (which is editable should you change your mind about the destination of your edits), and when you're happy with the path, you hit enter to actually save the file.
