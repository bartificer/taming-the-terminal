...
In <<ttt24.adoc#ttt24,the previous instalment>> we focused on the lowest of the four layers, the link layer, and looked at how ethernet and ARP work.
...
If your computer determines that the destination IP address is on the local network, then it looks up the MAC address for the destination IP using ARP (as described in <<ttt24.adoc#ttt24,the previous instalment>>), and sends the packet directly to its destination using ethernet.
