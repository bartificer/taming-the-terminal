...
In xref:ttt23[part 23 of n], we took a big-picture look at how TCP/IP networking works.
...
In xref:ttt24[the previous instalment], we focused on the lowest of the four layers, the link layer, and looked at how Ethernet and ARP work.
...
2+|Scan the QR code to listen on a different device

a|image::qrcodes/TTT_25a.png[QR code 25a]
a|image::qrcodes/TTT_25b.png[QR code 25b]
...
If your computer determines that the destination IP address is on the local network, then it looks up the MAC address for the destination IP using ARP (as described in xref:ttt24[the previous instalment]) and sends the packet directly to its destination using Ethernet.
