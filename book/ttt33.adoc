...
So far we've learned how to xref:ttt29[securely execute terminal commands on remote computers], how to xref:ttt31[securely copy files across the network using SSH], how to xref:ttt30[add both security and convenience] to both those operations with SSH key pairs, and how to xref:ttt32[tunnel just about anything through SSH].
...
image::qrcodes/TTT_33.png[QR code, align='left']
