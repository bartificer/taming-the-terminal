[[ttt37]]
= TTT Part 37of 1 {longdash} SSH Agents
...
ifndef::backend-pdf[]
pass:m,a[<audio controls='controls' src="https://media.blubrry.com/tamingtheterminal/archive.org/download/ttt-37-ssh-agents_202004/TTT_37_SSH_Agents.mp3">Your browser does not support HTML 5 audio</audio>]
endif::[]
...
You can also
https://media.blubrry.com/tamingtheterminal/archive.org/download/ttt-37-ssh-agents_202004/TTT_37_SSH_Agents.mp3[download the MP3]
****
...
The core SSH libraries are aware of SSH Agents and can communicate with them, so the caching they provide is available to all the SSH-related terminal commands like `ssh`, `scp`, `sftp`, as well as to any GUI apps footnote:[Here`'s a link to the SSH Client myself and Allison recommended during the recording of the matching podcast for this blog post: https://itunes.apple.com/us/app/ssh-shell/id981765152?mt=12] that make use of the appropriate SSH APIs, for example, both Transmit and SequelPro have SSH Agent support.
