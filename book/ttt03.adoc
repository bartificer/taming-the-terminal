...
In the xref:ttt01[first instalment], we started with the 40,000ft view, looking at what command shells are, and why they're still relevant in today's GUI-dominated world.
In the xref:ttt02[second instalment] we looked at OS X's Terminal.app, the anatomy of the Bash command prompt, and the anatomy of a Unix/Linux command.
...
image::qrcodes/TTT_03.png[QR code]
